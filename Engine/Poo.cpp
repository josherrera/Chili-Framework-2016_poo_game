...
void Poo::Init(int in_x, int in_y, int in_x_dir, int in_y_dir)
{
...
void Poo::Update()
{
	...
	if ((xdir == 1) && ((x + width + vel) <= Graphics::ScreenWidth))
	{
		x = x + vel;
	}
	else if ((xdir == 1) && ((x + width + vel) >= Graphics::ScreenWidth))
	{
		xdir = 0;
		x = x - vel;
	}
	...
	//Up and Down control 
	if ((ydir == 1) && ((y + height + vel) <= Graphics::ScreenHeight))
	{
		y = y + vel;
	}
	else if ((ydir == 1) && ((y + height + vel) >= Graphics::ScreenHeight))
	{
		ydir = 0;
		y = y - vel;
	}
...
void Poo::PooCollision(const Dude& dude)
{
	assert(initialized == true);
	const int dudeLeft = dude.GetX();
	const int dudeRight = dude.GetX() + Dude::GetWidth(); 
	const int dudeTop = dude.GetY();
