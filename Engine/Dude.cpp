...
void Dude::EnableMove(MainWindow & wnd)
{
	if (wnd.kbd.KeyIsPressed(VK_RIGHT) && ((x + width + vel) <= Graphics::ScreenWidth))
	{
		x = x + vel;
	}
	if (wnd.kbd.KeyIsPressed(VK_LEFT) && ((x - vel) >= 0))
	{
		x = x - vel;
	}
	if (wnd.kbd.KeyIsPressed(VK_UP) && ((y - vel) >= 0))
	{
		y = y - vel;
	}
	if (wnd.kbd.KeyIsPressed(VK_DOWN) && ((y + height + vel) <= Graphics::ScreenHeight))
	{
		y = y + vel;
	}
}
